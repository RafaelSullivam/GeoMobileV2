...
import { Text, View, TextInput, TouchableOpacity, ScrollView, Alert, Image, ActivityIndicator, FlatList, Modal } from 'react-native';
...
export default function FrascoAreia() {
    api = Conexao.api
    ...
    const SaveBanco = () => {
        console.log(pesofrascoantes)
        db.transaction((tx) => {
            tx.executeSql('SELECT * FROM FrascoAreia WHERE Processo=? and Codigo_C1 = ?', [processo,Codigo_C1], (tx, results) => {
                var temp = [];
                console.log(results)
...
console.log(ensaio)
        if (ensaio == "P1") {
            ...
            UpdateQuery1(estaca, pesofrascoantes.toString().replace(".", ","), pesofrascodepois.toString().replace(".", ","), pesoareiacone.toString().replace(".", ","), densidadeareia.toString().replace(".", ","), furo.toString().replace(".", ","), norte, este, cota, camada.toString().replace(".", ","), ncapsula.toString().replace(".", ","), capsulasoloagua.toString().replace(".", ","), capsulasolo.toString().replace(".", ","), capsula.toString().replace(".", ","), umidadeotima.toString().replace(".", ","), pesomaterialfuro.toString().replace(".", ","), profundidadefuro.toString().replace(".", ","), currentDate, currentHora)


        }
        if (ensaio == "P2") {

            UpdateQuery2(estaca, pesofrascoantes.toString().replace(".", ","), pesofrascodepois.toString().replace(".", ","), pesoareiacone.toString().replace(".", ","), densidadeareia.toString().replace(".", ","), furo.toString().replace(".", ","), norte, este, cota, camada.toString().replace(".", ","), ncapsula.toString().replace(".", ","), capsulasoloagua.toString().replace(".", ","), capsulasolo.toString().replace(".", ","), capsula.toString().replace(".", ","), umidadeotima.toString().replace(".", ","), pesomaterialfuro.toString().replace(".", ","), profundidadefuro.toString().replace(".", ","), currentDate, currentHora)
        }
        if (ensaio == "P3") {

            UpdateQuery3(estaca, pesofrascoantes.toString().replace(".", ","), pesofrascodepois.toString().replace(".", ","), pesoareiacone.toString().replace(".", ","), densidadeareia.toString().replace(".", ","), furo.toString().replace(".", ","), norte, este, cota, camada.toString().replace(".", ","), ncapsula.toString().replace(".", ","), capsulasoloagua.toString().replace(".", ","), capsulasolo.toString().replace(".", ","), capsula.toString().replace(".", ","), umidadeotima.toString().replace(".", ","), pesomaterialfuro.toString().replace(".", ","), profundidadefuro.toString().replace(".", ","), currentDate, currentHora)
        }
        if (ensaio == "P4") {

            UpdateQuery4(estaca, pesofrascoantes.toString().replace(".", ","), pesofrascodepois.toString().replace(".", ","), pesoareiacone.toString().replace(".", ","), densidadeareia.toString().replace(".", ","), furo.toString().replace(".", ","), norte, este, cota, camada.toString().replace(".", ","), ncapsula.toString().replace(".", ","), capsulasoloagua.toString().replace(".", ","), capsulasolo.toString().replace(".", ","), capsula.toString().replace(".", ","), umidadeotima.toString().replace(".", ","), pesomaterialfuro.toString().replace(".", ","), profundidadefuro.toString().replace(".", ","), currentDate, currentHora)
        }
        if (ensaio == "P5") {
            UpdateQuery5(estaca, pesofrascoantes.toString().replace(".", ","), pesofrascodepois.toString().replace(".", ","), pesoareiacone.toString().replace(".", ","), densidadeareia.toString().replace(".", ","), furo.toString().replace(".", ","), norte, este, cota, camada.toString().replace(".", ","), ncapsula.toString().replace(".", ","), capsulasoloagua.toString().replace(".", ","), capsulasolo.toString().replace(".", ","), capsula.toString().replace(".", ","), umidadeotima.toString().replace(".", ","), pesomaterialfuro.toString().replace(".", ","), profundidadefuro.toString().replace(".", ","), currentDate, currentHora)
        }
    ...
    const UpdateQuery2 = (estaca, pesofrascoantes, pesofrascodepois, pesoareiacone, densidadeareia, furo, norte, este, cota, camada, ncapsula, capsulasoloagua, capsulasolo, capsula, umidadeotima, pesomaterialfuro, profundidadefuro, currentDate, currentHora) => {
        ...
        db.transaction(
            (tx) => {
                tx.executeSql('UPDATE FrascoAreia SET  estaca2 =?, pesofrascoantes2 =?,pesofrascoantes2=?,pesofrascodepois2=?,pesoareiacone2=?,densidadeareia2=?,furo2=?,norte2=?,este2=?,cota2=?,camada2=?,ncapsula2=?,capsulasoloagua2=?,capsulasolo2=?,capsula2=?,umidadeotima2=?,pesomaterialfuro2=?,profundidadefuro2=?,Data_C2=?, Hora_C2=? WHERE processo=? and Codigo_C1 =?', [estaca, pesofrascoantes, pesofrascoantes, pesofrascodepois, pesoareiacone, densidadeareia, furo, norte, este, cota, camada, ncapsula, capsulasoloagua, capsulasolo, capsula, umidadeotima, pesomaterialfuro, profundidadefuro, currentDate, currentHora, processo, Codigo_C1], (tx, results) => {
                    console.log('Results', results.rowsAffected);
                    if (results.rowsAffected > 0) {
                        mensagemsalvaroffline()
                    } else alert('Registration Failed');
        ...
        db.transaction((tx) => {
            tx.executeSql('SELECT * FROM FrascoAreia WHERE processo = ? and Codigo_C1 = ?', [processo, Codigo_C1], (tx, results) => {
                ...
                if (results.rows.length > 0) {
                    ...
                    add2(results.rows.item(0).estaca3, results.rows.item(0).pesofrascoantes3, results.rows.item(0).pesofrascodepois3, results.rows.item(0).pesoareiacone3, results.rows.item(0).densidadeareia3, results.rows.item(0).furo3, results.rows.item(0).norte3, results.rows.item(0).este3, results.rows.item(0).cota3, results.rows.item(0).profundidadefuro3, results.rows.item(0).Data_C3, results.rows.item(0).Hora_C3, results.rows.item(0).camada3, results.rows.item(0).ncapsula3, results.rows.item(0).capsula3, results.rows.item(0).capsulasoloagua3, results.rows.item(0).capsulasolo3, results.rows.item(0).umidadeotima3, results.rows.item(0).pesomaterialfuro3, results.rows.item(0).estaca4, results.rows.item(0).pesofrascoantes4, results.rows.item(0).pesofrascodepois4, results.rows.item(0).pesoareiacone4, results.rows.item(0).densidadeareia4, results.rows.item(0).furo4, results.rows.item(0).norte4, results.rows.item(0).este4, results.rows.item(0).cota4, results.rows.item(0).profundidadefuro4, results.rows.item(0).Data_C4, results.rows.item(0).Hora_C4, results.rows.item(0).camada4, results.rows.item(0).ncapsula4, results.rows.item(0).capsula4, results.rows.item(0).capsulasoloagua4, results.rows.item(0).capsulasolo4, results.rows.item(0).umidadeotima4, results.rows.item(0).pesomaterialfuro4, results.rows.item(0).estaca5, results.rows.item(0).pesofrascoantes5, results.rows.item(0).pesofrascodepois5, results.rows.item(0).pesoareiacone5, results.rows.item(0).densidadeareia5, results.rows.item(0).furo5, results.rows.item(0).norte5, results.rows.item(0).este5, results.rows.item(0).cota5, results.rows.item(0).profundidadefuro5, results.rows.item(0).Data_C5, results.rows.item(0).Hora_C5, results.rows.item(0).camada5, results.rows.item(0).ncapsula5, results.rows.item(0).capsula5, results.rows.item(0).capsulasoloagua5, results.rows.item(0).capsulasolo5, results.rows.item(0).umidadeotima5, results.rows.item(0).pesomaterialfuro5)

                }
            });
        });
    }
    async function add2(estaca3, pesofrascoantes3,pesofrascodepois3, pesoareiacone3, densidadeareia3, furo3, norte3, este3, cota3, profundidadefuro3, Data_C3, Hora_C3, camada3, ncapsula3, capsula3, capsulasoloagua3, capsulasolo3, umidadeotima3, pesomaterialfuro3, estaca4, pesofrascoantes4,pesofrascodepois4, pesoareiacone4, densidadeareia4, furo4, norte4, este4, cota4, profundidadefuro4, Data_C4, Hora_C4, camada4, ncapsula4, capsula4, capsulasoloagua4, capsulasolo4, umidadeotima4, pesomaterialfuro4, estaca5, pesofrascoantes5,pesofrascodepois5, pesoareiacone5, densidadeareia5, furo5, norte5, este5, cota5, profundidadefuro5, Data_C5, Hora_C5, camada5, ncapsula5, capsula5, capsulasoloagua5, capsulasolo5, umidadeotima5, pesomaterialfuro5) {

        const obj = { conf: 'b', processo, Codigo_C1, estaca3: estaca3, pesofrascoantes3: pesofrascoantes3,pesofrascodepois3:pesofrascodepois3, pesoareiacone3: pesoareiacone3, densidadeareia3: densidadeareia3, furo3: furo3, norte3: norte3, este3: este3, cota3: cota3, profundidadefuro3: profundidadefuro3, Data_C3: Data_C3, Hora_C3: Hora_C3, camada3: camada3, ncapsula3: ncapsula3, capsula3: capsula3, capsulasoloagua3: capsulasoloagua3, capsulasolo3: capsulasolo3, umidadeotima3: umidadeotima3, pesomaterialfuro3: pesomaterialfuro3, estaca4: estaca4, pesofrascoantes4: pesofrascoantes4,pesofrascodepois4:pesofrascodepois4,pesofrascodepois5:pesofrascodepois5, pesoareiacone4: pesoareiacone4, densidadeareia4: densidadeareia4, furo4: furo4, norte4: norte4, este4: este4, cota4: cota4, profundidadefuro4: profundidadefuro4, Data_C4: Data_C4, Hora_C4: Hora_C4, camada4: camada4, ncapsula4: ncapsula4, capsula4: capsula4, capsulasoloagua4: capsulasoloagua4, capsulasolo4: capsulasolo4, umidadeotima4: umidadeotima4, pesomaterialfuro4: pesomaterialfuro4, estaca5: estaca5, pesofrascoantes5: pesofrascoantes5, pesoareiacone5: pesoareiacone5, densidadeareia5: densidadeareia5, furo5: furo5, norte5: norte5, este5: este5, cota5: cota5, profundidadefuro5: profundidadefuro5, Data_C5: Data_C5, Hora_C5: Hora_C5, camada5: camada5, ncapsula5: ncapsula5, capsula5: capsula5, capsulasoloagua5: capsulasoloagua5, capsulasolo5: capsulasolo5, umidadeotima5: umidadeotima5, pesomaterialfuro5: pesomaterialfuro5,login:log.login };

        if (processo > 0) {
            const res = await axios.post(this.api + 'SalvarFrascodeAreiaB.php', obj);
            console.log(this.api + 'SalvarFrascodeAreiaB.php', obj)
            if (res.data.success === true) {
                mensagemsalvar()
    ...
    const SalvarBanco = () => {
        ...
        db.transaction((tx) => {
                tx.executeSql(`insert into FrascoAreia ( processo,Codigo_C1,trecho ,estudo ,subtrecho ,energiacomp ,  estaca1 , pesofrascoantes1 ,pesofrascodepois1,pesoareiacone1,densidadeareia1,furo1,norte1,este1,cota1,camada1,ncapsula1,capsulasoloagua1,capsulasolo1,capsula1,umidadeotima1,pesomaterialfuro1,profundidadefuro1,Data_C1, Hora_C1,estaca2  ,pesofrascoantes2,pesofrascodepois2,pesoareiacone2,densidadeareia2,furo2,norte2,este2,cota2,camada2,ncapsula2,capsulasoloagua2,capsulasolo2,capsula2,umidadeotima2,pesomaterialfuro2,profundidadefuro2,Data_C2, Hora_C2,estaca3 , pesofrascoantes3 ,pesofrascodepois3,pesoareiacone3,densidadeareia3,furo3,norte3,este3,cota3,camada3,ncapsula3,capsulasoloagua3,capsulasolo3,capsula3,umidadeotima3,pesomaterialfuro3,profundidadefuro3,Data_C3, Hora_C3,estaca4  ,pesofrascoantes4,pesofrascodepois4,pesoareiacone4,densidadeareia4,furo4,norte4,este4,cota4,camada4,ncapsula4,capsulasoloagua4,capsulasolo4,capsula4,umidadeotima4,pesomaterialfuro4,profundidadefuro4,Data_C4, Hora_C4 ) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [processo,Codigo_C1,trecho, estudo, subtrecho, energiacomp, estaca, pesofrascoantes, pesofrascodepois, pesoareiacone, densidadeareia, furo, norte, este, cota, camada, ncapsula, capsulasoloagua, capsulasolo, capsula, umidadeotima, pesomaterialfuro, profundidadefuro, currentDate, currentHora,estaca2  ,pesofrascoantes2,pesofrascodepois2,pesoareiacone2,densidadeareia2,furo2,norte2,este2,cota2,camada2,ncapsula2,capsulasoloagua2,capsulasolo2,capsula2,umidadeotima2,pesomaterialfuro2,profundidadefuro2,Data_C2, Hora_C2,estaca3 , pesofrascoantes3 ,pesofrascodepois3,pesoareiacone3,densidadeareia3,furo3,norte3,este3,cota3,camada3,ncapsula3,capsulasoloagua3,capsulasolo3,capsula3,umidadeotima3,pesomaterialfuro3,profundidadefuro3,Data_C3, Hora_C3,estaca4 ,pesofrascoantes4,pesofrascodepois4,pesoareiacone4,densidadeareia4,furo4,norte4,este4,cota4,camada4,ncapsula4,capsulasoloagua4,capsulasolo4,capsula4,umidadeotima4,pesomaterialfuro4,profundidadefuro4,Data_C4, Hora_C4], (tx, results) => {
    ...
    const UpdateQueryc5 = () => {
        ...
        db.transaction(
            (tx) => {
                tx.executeSql('UPDATE FrascoAreia SET  estaca5 =?, pesofrascoantes5 =?,pesofrascoantes5=?,pesofrascodepois5=?,pesoareiacone5=?,densidadeareia5=?,furo5=?,norte5=?,este5=?,cota5=?,camada5=?,ncapsula5=?,capsulasoloagua5=?,capsulasolo5=?,capsula5=?,umidadeotima5=?,pesomaterialfuro5=?,profundidadefuro5=?,Data_C5=?, Hora_C5=? WHERE processo=? and Codigo_C1 =?', [estaca5, pesofrascoantes5, pesofrascoantes5, pesofrascodepois5, pesoareiacone5, densidadeareia5, furo5, norte5, este5, cota5, camada5, ncapsula5, capsulasoloagua5, capsulasolo5, capsula5, umidadeotima5, pesomaterialfuro5, profundidadefuro5, Data_C5, Hora_C5, processo, Codigo_C1], (tx, results) => {
                    console.log('Results', results.rowsAffected);
                    if (results.rowsAffected > 0) {
                        mensagemsalvaroffline()
                    } else alert('Registration Failed');
    ...
    let DeleteBancoSQlite = () => {
        try {
          db.transaction((tx) => {
            tx.executeSql(
              'DROP TABLE IF EXISTS FrascoAreia',
              (tx, results) => {
                console.log('Results', results.rowsAffected);
              }
            );
