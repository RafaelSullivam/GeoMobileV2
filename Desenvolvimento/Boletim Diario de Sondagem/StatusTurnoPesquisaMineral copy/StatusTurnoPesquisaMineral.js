...
export default function StatusTurnoPesquisaMineral(props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists StatusTurnoPesquisaMineral (IdTurno text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into StatusTurnoPesquisaMineral ( IdTurno) values ('${idturno}' )`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    let DeleteBancoSQlite = () => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM  StatusTurnoPesquisaMIneral where IdTurno =?',
                [idturno],
                (tx, results) => {
                    ...
                    if (results.rowsAffected > 0) {
                        Alert.alert(
                            'Successo',
                            'Id Status Turno Deletada com Sucesso',
                            [
                        ...
                        alert('Por Favor Inserir um Id Status Turno Valida');
                    }
                }
            );
        });
    ...
    return (
        ...
        <ScrollView style={styles.scrollView} >
            <View style={styles.container}>
                <Text style={styles.text}>ID Status do Turno</Text>
                <TextInput style={styles.input}
                    onChangeText={setIdTurno}
                    value={idturno}
                    placeholder="ID Status do Turno"
                >
                </TextInput>
                    ...
                    renderItem={({ item }) => {
                        return (
                            <View style={styles.Tasks}>
                                ...
                                <Text style={styles.labelperfuracao}>ID Status do Turno:</Text>
                                <Text
