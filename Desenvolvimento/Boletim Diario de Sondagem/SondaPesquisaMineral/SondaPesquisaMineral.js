...
export default function SondaPesquisaMineral(props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists SondaPesquisaMineral (IdSonda text, Modelo text, Fabricante text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into SondaPesquisaMineral ( IdSonda , Modelo , Fabricante) values ('${idsonda}','${modelo}','${fabricante}' )`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    let DeleteBancoSQlite = () => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM  SondaPesquisaMIneral where IdSonda =?',
                [idsonda],
                (tx, results) => {
