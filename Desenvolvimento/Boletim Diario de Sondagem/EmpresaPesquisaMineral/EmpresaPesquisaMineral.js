...
export default function EmpresaPesquisaMineral(props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists EmpresaPesquisaMineral (IdEmpresa text, Categoria text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into EmpresaPesquisaMineral ( IdEmpresa , Categoria) values ('${idempresa}','${categoria}' )`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    const BuscaBancoSQlite = () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('SELECT * FROM EmpresaPesquisaMIneral', null, (tx, results) => {
                var temp = [];
    ...
    let DeleteBancoSQlite = () => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM  EmpresaPesquisaMIneral where IdEmpresa =?',
                [idempresa],
                (tx, results) => {
