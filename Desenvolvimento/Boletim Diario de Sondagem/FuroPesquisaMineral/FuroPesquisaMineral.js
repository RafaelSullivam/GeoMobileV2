...
export default function FuroPesquisaMineral(props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists FuroPesquisaMineral (IdFuro text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into FuroPesquisaMineral ( IdFuro) values ('${idfuro}' )`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    let DeleteBancoSQlite = () => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM  FuroPesquisaMIneral where IdFuro =?',
                [idfuro],
                (tx, results) => {
