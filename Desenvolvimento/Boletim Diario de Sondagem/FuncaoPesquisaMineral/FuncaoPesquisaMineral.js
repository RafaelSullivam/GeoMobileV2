...
export default function FuncaoPesquisaMineral(props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists FuncaoPesquisaMineral (IdFuncao text, Categoria text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into FuncaoPesquisaMineral ( IdFuncao , Categoria) values ('${idfuncao}','${categoria}' )`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    let DeleteBancoSQlite = () => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM  FuncaoPesquisaMIneral where IdFuncao =?',
                [idfuncao],
                (tx, results) => {
