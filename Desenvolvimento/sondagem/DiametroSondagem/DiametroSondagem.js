...
export default function DiametroSondagem(props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists DiametroSondagem (IdDiamentro text, Serie text, mm text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into DiametroSondagem ( IdDiamentro , Serie , mm) values ('${iddiametro}','${serie}','${mm}' )`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    let DeleteBancoSQlite = () => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM  DiametroSondagem where IdSonda =?',
                [iddiametro],
                (tx, results) => {
