...
export default function AlmoxarifadoSondagem({ route, navigation }, props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists BoletimSondagem (IdFuro text, DIP text, AZ text, Tipo text, Data text, Sonda text , Turno text, StatusBoletim text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        ...
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into BoletimSondagem ( IdFuro, AZ, DIP, Tipo , StatusBoletim) values ('${quantidade}','${ate}','${av}','${insumo}', '${'Aberto'}')`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    let DeleteBancoSQlite = () => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM   BoletimSondagem where IdFuro =?',
                [quantidade],
                (tx, results) => {
