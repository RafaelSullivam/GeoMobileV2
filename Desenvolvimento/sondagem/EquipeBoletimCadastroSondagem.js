...
export default function EquipeBoletimCadastroSondagem ({ route, navigation }, props) { 
    api2 = Conexao.api
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists EquipeSondagem (id INTEGER PRIMARY KEY AUTOINCREMENT, idFuro text, Matricula text, Nome text, Funcao text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        ...
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into EquipeSondagem ( Matricula, Nome, Funcao,idFuro) values ('${matricula}','${nome}','${funcao}','${processo}')`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    const BuscaBancoSQlite1 = () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('SELECT * FROM EquipeSondagem where IdFuro = ?', [processo], (tx, results) => {
                var temp = [];
                for (let i = 0; i < results.rows.length; ++i)
                    temp.push(results.rows.item(i));
                setTask1(temp);
    ...
    let DeleteBancoSQlite = (id) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM   EquipeSondagem where id =?',
                [id],
                (tx, results) => {
    ...
    let DeleteBancoSQlite1 = (furo) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DROP TABLE IF EXISTS EquipeSondagem '
    ...
    async function add() {
        const obj = { idfuro, nome , matricula , funcao };


        const res = await axios.post(this.api2 + 'SalvarEquipeSondagem.php', obj);
