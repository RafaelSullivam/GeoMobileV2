...
const IndeformadaTubo = (props) => {
    ...
    async function Buscar() {
        ...
        setBloquear(false)
        Limpar()
        const res = await axios.get(this.api2 + 'SalvarAmostraIndeformadaTUBO.php?numeroAmostra=' + numeroAmostra);
        ...
        if (res.data.success === true) {
            ...
            setProfundidadeTopo(res.data.result[0].profundidadeTopo)
            setProfundidadeBase(res.data.result[0].profundidadeBase)
            setidentificacaoAmostra(res.data.result[0].identificacaoAmostra)
            setCota(res.data.result[0].cota)
            setObservacao(res.data.result[0].observacao)
            setTipoAmostra(res.data.result[0].tipoAmostra)
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists AmostraIndeformadaTubo ( cliente text , numeroAmostra text , obra text,   local text ,   dataa text, identificacao text,  coordenadax  text, coordenaday text,  cota text,   tipoAmostra text,   comprimento text,   observacao text);"
                );
    ...
    const gerarBancoSQlite1 = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists AmostraIndeformadaTuboModal  ( id INTEGER PRIMARY KEY AUTOINCREMENT, numeroAmostra text , profundidadeTopo text , profundidadeBase text , identificacaoAmostra text   );"
                );
                console.log(" Nova tabela gerada")
            });
    };
    ...
    const SalvarBancoSQLite = () => {
        ...
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into AmostraIndeformadaTubo  (cliente , numeroAmostra , obra , local ,  dataa , identificacao , coordenadax , coordenaday , cota ,tipoAmostra, comprimento , observacao ) values ('${cliente}','${numeroAmostra}','${obra}','${local}','${dataa}','${identificacao}','${coordenadax}','${coordenaday}','${cota}','${tipoAmostra}','${comprimento}','${observacao}' )`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    const SalvarBancoSQLite1 = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into AmostraIndeformadaTuboModal ( numeroAmostra , profundidadeTopo , profundidadeBase , identificacaoAmostra) values ('${numeroAmostra}','${profundidadeTopo}','${profundidadeBase}','${identificacaoAmostra}'  )`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    const BuscaBancoSQl = () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('SELECT * FROM AmostraIndeformadaTubo WHERE numeroAmostra =?', [numeroAmostra], (tx, results) => {
                ...
                if (temp > 0) {
                    ...
                    setObra(results.rows.item(0).obra)
                    ...
                    setTipoAmostra(results.rows.item(0).tipoAmostra)
    ...
    async function add1(lista) {
        ...
        for (let i = 0; i < lista.length; ++i) {
            results.push(
                SalvarListaDinamica(lista[i].numeroAmostra, lista[i].cliente, lista[i].local, lista[i].obra, lista[i].identificacao, lista[i].coordenadax, lista[i].coordenaday, lista[i].profundidadeTopo, lista[i].profundidadeBase, lista[i].identificacaoAmostra, lista[i].cota, lista[i].tipoAmostra, lista[i].comprimento, lista[i].dataa, lista[i].observacao )
            );
        }
    ...
    async function SalvarListaDinamica(numeroAmostra, cliente, local, obra, identificacao, coordenadax, coordenaday, profundidadeTopo, profundidadeBase, identificacaoAmostra, cota, tipoAmostra, comprimento, dataa, observacao) {
        const obj = { numeroAmostra, cliente, local, obra, identificacao, coordenadax, coordenaday, profundidadeTopo, profundidadeBase, identificacaoAmostra, cota, tipoAmostra, tipoAmostra, comprimento, dataa, observacao };

        try {
            const res = await axios.post(api2 + 'SalvarAmostraIndeformadaTUBO.php', obj);
            if (res.data.success === true) {
                console.log("Finalizada");
            }
        } catch (error) {
    ...
    const BuscaBancoSQl1 = () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('SELECT * FROM AmostraIndeformadaTuboModal WHERE numeroAmostra =?', [numeroAmostra], (tx, results) => {
                var temp = [];
                for (let i = 0; i < results.rows.length; ++i)
                    temp.push(results.rows.item(i));
                setTask(temp);
                console.log('Tabela OFF AmostraIndeformadaTubo:', temp)
            });
    ...
    let DeleteBancoSQlite1 = (id) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM  AmostraIndeformadaTubo where id =?',
                [id],
                (tx, results) => {
    ...
    let DeleteBancoSQlite = () => {
        db.transaction((tx) => {
            tx.executeSql(
                'DROP TABLE IF EXISTS  AmostraIndeformadaTubo ',
        ...
        <ScrollView>
            ...
            >
                ...
                <View style={estilo.modal1} >
                    ...
                    <Text style={styles.text}> Profundidade Topo: </Text>
                    <TextInput
                    ...
                    <Text style={styles.text}> Identificação da Amostrae: </Text>
                    <TextInput
            ...
            <View style={styles.container}>
                ...
                <View style={styles.container}>
                    ...
                    <Text style={styles.text}> Obra: </Text>
                    <TextInput
                ...
                <View style={styles.container}>
                    ...
                    <Text style={styles.text}> Identificação Do Furo: </Text>
                    <TextInput
                ...
                <View style={styles.container}>
                    ...
                    <Text style={styles.text}> Cota: </Text>
                    <TextInput
                ...
                <View style={styles.container}>
                    ...
                    <Text style={styles.text}> Comprimento do Amostrador: </Text>
                    <TextInput
                        ...
                        renderItem={({ item }) => {
                            return (
                                <View style={styles.Tasks}>
                                    ...
                                    <Text style={styles.labelperfuracao}>Profundidade Topo:</Text>
                                    <Text
                                    ...
                                    <Text style={styles.labelperfuracao}>Profundidade Base:</Text>
                                    <Text
                                        style={styles.DescriptionTask}
                                    >
                                        {item.profundidadeBase}
                                    </Text>

                                    <Text style={styles.labelperfuracao}>Identificação da Amostra:</Text>
                                    <Text
                                        style={styles.DescriptionTask}
                                    >
                                        {item.identificacaoAmostra}
                                    </Text>
                ...
                >
                    <Text style={styles.text}>Tipo Tubo  </Text>
...
export default IndeformadaTubo;