...
export default function ManobraBoletimCadastroSondagem({ route, navigation }, props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists ManobraSondagem(id INTEGER PRIMARY KEY AUTOINCREMENT, idFuro text, De text, Ate text, Av text, Rec text , RecPorCento text, Litologia text, Diametro text , TempoCorte text, LuvaSerie text, CoroaSerie text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        ...
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into ManobraSondagem (idFuro, De, Ate, Av, Rec, Litologia, Diametro, TempoCorte, LuvaSerie, CoroaSerie, RecPorCento) values ('${processo}','${de}','${ate}','${av}','${rec}', '${litologia}', '${diametro}', '${tempocorte}', '${luvanserie}', '${coroanserie}', '${recporcento}')`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    const BuscaBancoSQlite = () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('SELECT * FROM LitologiaSondagem ', null, (tx, results) => {
                var temp = [];
    ...
    const BuscaBancoSQlite2 = () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('SELECT * FROM DiametroSondagem', null, (tx, results) => {
                var temp = [];
                for (let i = 0; i < results.rows.length; ++i)
                    temp.push(results.rows.item(i));
                setTask2(temp);
    ...
    let DeleteBancoSQlite = (id) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM   ManobraSondagem where id =?',
                [id],
                (tx, results) => {
    ...
    let DeleteBancoSQlite1 = (furo) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DROP TABLE IF EXISTS ManobraSondagem'
    ...
    async function add() {
        const obj = { idfuro, ate , av , de , diametro , litologia , coroanserie , luvanserie , rec ,  recporcento , tempocorte };


        const res = await axios.post(this.api2 + 'SalvarManobraSondagem.php', obj);
