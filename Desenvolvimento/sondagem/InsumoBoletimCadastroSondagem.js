...
export default function InsumoBoletimCadastroSondagem({ route, navigation }, props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists InsumosSondagem (id INTEGER PRIMARY KEY AUTOINCREMENT, idFuro text, Insumo text, Quant text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        ...
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into InsumosSondagem( Insumo, Quant, idFuro) values ('${insumo}','${quantidade}','${processo}')`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    const BuscaBancoSQlite1 = () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('SELECT * FROM InsumosSondagem', null, (tx, results) => {
                var temp = [];
                for (let i = 0; i < results.rows.length; ++i)
                    temp.push(results.rows.item(i));
                setTask1(temp);
    ...
    let DeleteBancoSQlite = (id) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM   InsumosSondagem where id =?',
                [id],
                (tx, results) => {
    ...
    let DeleteBancoSQlite1 = (furo) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DROP TABLE IF EXISTS InsumosSondagem'
    ...
    async function add() {
        const obj = { idfuro, insumo , quantidade  };
    
    
        const res = await axios.post(this.api2 + 'SalvarInsumoSondagem.php', obj);
