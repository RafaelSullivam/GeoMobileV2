...
export default function NewAtivisdadeBoletimCadastroSondagem({ route, navigation }, props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists AtividadesBoletimSondagem (id INTEGER PRIMARY KEY AUTOINCREMENT, idFuro text, Atividade text, De text, Ate text);"
                );
    ...
    const SalvarBancoSQLite = () => {
        ...
        db.transaction(
            (tx) => {
                tx.executeSql(`insert into AtividadesBoletimSondagem  (idFuro, Atividade, De, Ate) values ('${processo}','${atividade}','${de}','${ate}')`, (er) => { console.log(er) }, (error) => { console.log(error) });
    ...
    const BuscaBancoSQlite = () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('SELECT * FROM AtividadeSondagem', null, (tx, results) => {
                var temp = [];
    ...
    let DeleteBancoSQlite = (id) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM   AtividadesBoletimSondagem where id =?',
                [id],
                (tx, results) => {
    ...
    async function add() {
        const obj = { idfuro, ate , atividade , de   };
    
    
        const res = await axios.post(this.api2 + 'SalvarAtividadesBoletimSondagem.php', obj);
