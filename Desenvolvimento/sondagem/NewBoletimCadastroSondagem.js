...
//StatusBoletim
export default function NewBoletimCadastroSondagem ({ route, navigation },props) {
    ...
    const gerarBancoSQlite = () => {
        db.transaction(
            (tx) => {
                tx.executeSql(
                    "create table if not exists StatusBoletimSondagem (id INTEGER PRIMARY KEY AUTOINCREMENT ,IdFuro text, Data text, Sonda text , Turno text, StatusBoletim text);"
                );
    ...
    const BuscaBancoSQlite1 = () => {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql('SELECT * FROM BoletimSondagem ',null, (tx, results) => {
                var temp = [];
                for (let i = 0; i < results.rows.length; ++i)
                    temp.push(results.rows.item(i));
                setTask1(temp);
    ...
    let DeleteBancoSQlite = (furo) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DELETE FROM   StatusBoletimSondagem where id =?',
                [furo],
                (tx, results) => {
    ...
    let DeleteBancoSQlite1 = (furo) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DROP TABLE IF EXISTS StatusBoletimSondagem'
    ...
    async function add() {
                const obj = { sonda , turno , statusfuro , selectedDate , idfuro };
        
        
                const res = await axios.post(this.api2 + 'SalvarStatustBoletimSondagem.php', obj);
                        ...
                        return (
                            <View style={styles.Tasks}>
                                ...
                                <Text
                                    ...
                                    onPress={() =>
                                         navigation.navigate('AtividadesBoletimSondagem',{ nprocesso: processo })||
